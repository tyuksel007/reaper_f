...
module Tilson = 
    ...
    let ema (prices: decimal[]) (period: int) (index: int) = 
        ...
        for i = index - 1 downto index - period + 1 do
            ema <- (prices.[i] * multiplier) + ema * (1.0m - multiplier)
        ema
    let ema2 (prices: decimal[]) (period: int) (index: int) = 
        printfn "Calculating EMA"
        if index - period + 1 < 0 then failwith "Not enough data to calculate EMA"
        ...
        try
            ...
            ema
        with
            | e -> failwith $"index = {index}, period = {period}  \n error: {e}"


    let foo (prices: decimal[]) (period: int) (volumeFactor: decimal) =
        printfn "Hello from Tilson module"
        ema2 prices period 80

    let calculateT3 (prices: decimal[]) (period: int) (volumeFactor: decimal) =
        printfn "Calculating T3"
