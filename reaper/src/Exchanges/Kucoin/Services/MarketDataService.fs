...
module MarketDataService = 
    ...
    let private get_max_200_candles(symbol: string) (from: float) (to_: float) (intervalInMins: int) = 
        async{
    ...
    let get_candles (symbol: string) (startTime: string) (endTime: Option<string>) (interval: int) = 
        asyncSeq{
            ...
            for i = 0 to count_of_iteration do
                ...
                let! iter_candles = get_max_200_candles symbol from next_to interval
                from <- next_to
                yield iter_candles
        }
